...
func open(paths []string) ([]core.WriteSyncer, func(), error) {
	writers := make([]core.WriteSyncer, 0, len(paths))
...
// using core.NewMultiWriteSyncer and core.Lock individually.
func CombineWriteSyncers(writers ...core.WriteSyncer) core.WriteSyncer {
	if len(writers) == 0 {
		return core.AddSync(ioutil.Discard)
	}
	return core.Lock(core.NewMultiWriteSyncer(writers...))
}
