...
type Logger struct {
	...
	development bool
	addCaller   bool
	onFatal     core.CheckWriteAction // default is WriteThenFatal
...
// New constructs a new Logger from the provided core.Core and Options. If
// the passed core.Core is nil, it falls back to using a no-op
// implementation.
//
...
func New(ccore core.Core, options ...Option) *Logger {
	...
	log := &Logger{
		core:        ccore,
		errorOutput: core.Lock(os.Stderr),
		addStack:    core.FatalLevel + 1,
...
func NewNop() *Logger {
	return &Logger{
		...
		errorOutput: core.AddSync(ioutil.Discard),
		addStack:    core.FatalLevel + 1,
		clock:       core.DefaultClock,
	}
}
...
func NewExample(options ...Option) *Logger {
	...
	core := core.NewCore(core.NewJSONEncoder(encoderCfg), os.Stdout, DebugLevel)
...
func (log *Logger) check(lvl core.Level, msg string) *core.CheckedEntry {
	...
	if lvl < core.DPanicLevel && !log.core.Enabled(lvl) {
		return nil
	}
	...
	case core.FatalLevel:
		...
		if onFatal == core.WriteThenNoop {
			onFatal = core.WriteThenFatal
		}
	...
	if log.addCaller {
		...
		ce.Entry.Caller = core.EntryCaller{
			Defined:  defined,
			PC:       frame.PC,
			File:     frame.File,
			Line:     frame.Line,
